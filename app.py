    ...
    for uploaded_file in uploaded_files:
        with st.spinner(f"Processing {uploaded_file.name}..."):
            ...
            if "error" in result:
                ...
                is_compliant = False
                ad_info = {"title": "N/A", "effective_date": "N/A"}
            else:
                summary = str(result)
...
# Display uploaded bulletins
st.divider()
st.subheader("🔍 View Uploaded Bulletins")

all_bulletins = fetch_all_bulletins()
df = pd.DataFrame(all_bulletins, columns=[
    "ID", "File", "Summary", "Aircraft", "ATA", "System", "Action", "Compliance",
    "Reason", "SB ID", "Group", "Is Compliant", "AD Title", "AD Effective Date"
])
...
st.dataframe(df[["File", "Aircraft", "ATA", "System", "Action", "Compliance", "SB ID", "Group", "Is Compliant", "AD Title", "AD Effective Date"]])

# Option to show full summary text
if st.checkbox("Show Full Summaries"):
    for i, row in df.iterrows():
        st.markdown(f"### 📄 {row['File']}")
        st.text_area("Structured Summary", row["Summary"], height=200, key=f"summary_{i}")
