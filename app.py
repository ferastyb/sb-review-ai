...
if uploaded_files:
    for uploaded_file in uploaded_files:
        with st.spinner(f"Processing {uploaded_file.name}..."):
            ...
            # Suggest relevant AD based on ATA + system
            suggested_ad = find_relevant_ad(f"{ata} {system}")

            summary = str(result)
            save_to_db(uploaded_file.name, summary, aircraft, ata, system, action, compliance, suggested_ad)
...
df = pd.DataFrame(fetch_all_bulletins(), columns=[
    "ID", "File", "Summary", "Aircraft", "ATA", "System", "Action", "Compliance", "Suggested_AD"
])
...
# Display table
st.dataframe(df[["File", "Aircraft", "ATA", "System", "Action", "Compliance", "Suggested_AD"]])

if st.checkbox("Show Full Summaries"):
    for i, row in df.iterrows():
        st.markdown(f"### 📄 {row['File']}")
        st.text_area("Summary", row["Summary"], height=200, key=f"raw_text_{i}")
